...
public interface Accessible {
	...
	default boolean canAccess(@NonNull AccessOptions options) {
		if(!isStatic() && options.staticAccess())
			return false;
		
		return switch(getAccessModifier()) {
		case PRIVATE -> options.sameClass();
		case PROTECTED -> options.sameClass() || options.fromSubclass();
		case PUBLIC -> true;
		case DEFAULT -> options.sameFile();
		};
	}
